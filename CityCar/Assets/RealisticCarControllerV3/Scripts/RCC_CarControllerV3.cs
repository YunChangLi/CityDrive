...
//		GetComponentInChildren<WheelCollider>().ConfigureVehicleSubsteps(10f, 1, 1);
	...
	void Inputs(){
		...
		case RCC_Settings.ControllerType.Keyboard:
                ...
                {
                    gasInput = Input.GetAxis(RCCSettings.verticalInput);                  //前进
                    brakeInput = Mathf.Clamp01(-Input.GetAxis(RCCSettings.verticalInput));//后退
                    if (!IsStop)
                    {
                        handbrakeInput = Input.GetKey(RCCSettings.handbrakeKB) ? 1f : 0f;     //刹车
                    }
                    else
                        handbrakeInput = 1;
                }
                ...
                {
                    gasInput = Input.GetAxis("youmen");                  //前进
                    brakeInput = Input.GetAxis("Back");//后退
                    if (!IsStop)
                    {
                        handbrakeInput = Input.GetAxis("Stop");
                    }//刹车
                    else
                        handbrakeInput = 1;
                ...
                {
                    gasInput = -Input.GetAxis("Youmen");
                    Debug.Log(Input.GetAxis("Youmen"));//前进
                    brakeInput =- Input.GetAxis("BoxBack");//后退
                    if (!IsStop)
                    {
