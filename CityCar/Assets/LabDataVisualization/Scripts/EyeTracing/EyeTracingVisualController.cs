    ...
    public class EyeTracingVisualController : VisualControllerBase
    {
        ...
        private IEnumerator EyeTracingVisualIEnumerator()
        {
            ...
            while (_iEyeTracingPos.GetEyeTracingPos() != null)
            {
                ...
                float x = (Mathf.Clamp(pos.x, 0.3f, 0.7f) - 0.3f) / (0.7f - 0.3f);
                float y = (Mathf.Clamp(pos.y, 0.3f, 0.7f) - 0.3f) / (0.7f - 0.3f);
                _config.EyetracingTextureObject.transform.position =
                    new Vector2(Mathf.Lerp(_config.EyetracingTextureObject.transform.position.x, x * Screen.width, 0.2f)
                        , Mathf.Lerp(_config.EyetracingTextureObject.transform.position.y, (1 - y) * Screen.height, 0.2f));
